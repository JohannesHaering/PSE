...
TEST(GetImage, valid){
    ...
    cv::Mat input = facade.getImage("/home/Documents/PSE/ImplementationPhase/build/testdata/readdir/testBMPRead.bmp", 64,64,1);
...
TEST(writeImage, valid)
{
	ImageFacade facade = ImageFacade();
    cv::Mat mat(64, 64, CV_32F, cv::Scalar(0, 0, 0));
    bool success = facade.writeImage("/home/Documents/PSE/ImplementationPhase/build/testdata/writedir/testBMPWrite.bmp", mat);
    EXPECT_EQ(true, success);
}

TEST(ImageGreyScale, valid){
  ImageFacade facade = ImageFacade();
  std::vector<float> image = facade.getImageGreyScale("/home/Documents/PSE/ImplementationPhase/build/testdata/readdir/testBMPRead.bmp", 64,64,1);
