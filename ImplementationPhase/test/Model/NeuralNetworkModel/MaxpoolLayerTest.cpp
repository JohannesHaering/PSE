...
TEST_F(MaxPoolTest, MaxPoolForwardTest)
{
    ...
    TESNOR(float) expectedOutput = TENSOR(float){{{{2,2}, {2,3}}}};
...
TEST_F(MaxPoolTest, MaxPoolForwardTest)
{
    ...
    TENSOR(float) output = maxLayer->backward(feedback);
    TESNOR(float) expectedOutput = TENSOR(float){{{{0,1 + 1/3,0,0}, {0,2 + 1/3,0,0}, {0,2 + 1/3,0,0}, {0,0,0,3}}}};
    for (int b = 0; b < output[0].size(); b++) {
        for (int z = 0; z < output[0].size(); z++) {
            for (int y = 0; y < output[0][0].size(); y++) {
                for (int x = 0; x < output[0][0][0].size(); x++) {
