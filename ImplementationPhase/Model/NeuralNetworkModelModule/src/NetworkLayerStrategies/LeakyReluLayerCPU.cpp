...
MatrixDefine::TENSOR(float) LeakyReLuLayerCPU::forward(MatrixDefine::TENSOR(float) input_data)
{
...
MatrixDefine::TENSOR(float) LeakyReLuLayerCPU::backprob(MatrixDefine::TENSOR(float) updates)
{
        ...
        for (int z = 0; z < feedback[0].size(); z++)
            for (int y = 0; y < feedback[0][0].size(); y++)
                for (int x = 0; x < feedback[0][0][0].size(); x++)
                    output_backward[b][z][y][x] = feedback[b][z][y][x] > 0 ? feedback[b][z][y][x] : 100 * feedback[b][z][y][z];
    return output;
}