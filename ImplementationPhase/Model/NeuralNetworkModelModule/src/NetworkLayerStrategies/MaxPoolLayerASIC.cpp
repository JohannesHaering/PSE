...
//    auto max0 = std::make_shared<ngraph::opset1::MaxPool>(arg0,ngraph::Strides({1,1,1}),ngraph::Shape({0,0}),ngraph::Shape({0,0}),ngraph::Shape({3}),ngraph::op::RoundingType::FLOOR);
//    max0->set_strides(ngraph::Strides({1,1,1}));
//    max0->set_pads_begin(ngraph::Shape({0,0}));
//   max0->set_adding_above(ngraph::Shape({0,0}));
