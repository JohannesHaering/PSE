...
MATRIX_2D(float) ConvolutionLayer::Convolute(MATRIX_3D(float) net, MATRIX_3D(float) filter, int stride, int padding)
{
  int newSizeX = net[0][0].size() - (filter[0][0].size() - 1);
  int newSizeY = net[0].size() - (filter[0].size() - 1);
  MATRIX_2D(float) featureMap = MATRIX_2D(float)(newSizeY, std::vector<float>(newSizeX, 0.0f));
...
TENSOR(float) ConvolutionLayer::forward(TENSOR(float) net)
{
...
MATRIX_3D(float) ConvolutionLayer::backpropError(MATRIX_3D(float) feedback)
{
  MATRIX_3D(float) D3_Result = MATRIX_3D(float)(net[0].size(), MATRIX_2D(float)(net[0][0].size(), std::vector<float>(net[0][0][0].size(), 0.0f)));
  for (int filter = 0; filter < numFilters; filter++)
