...
\textbf{MFR013} \hypertarget{MFR013}& \textbf{Layerimplementation}\\
& Tested with: \hyperlink{T010}{T010} Implements: \hyperlink{MAC010}{MAC010}\\
...
& If selected, for each image which has been selected by the user, the classification/ detection output will be stored as described in \hyperlink{PD050}{PD050} and \hyperlink{PD060}{PD060}. Each output file is saved in the same folder as the input file. If a camera is chosen as input, the output will be stored in a \grqq camera\grqq\ folder\\
...
The user has the options \grqq Single Image\grqq, \grqq Image Folder\grqq, \grqq Camera\grqq\ and \grqq File with Image paths\grqq. Then the file explorer opens and the user can choose the input accordingly to the chosen kind of input.\\
The user can check if he wants to save all results and show all results.\\
The user clicks on the button \grqq Chose neural network/s\grqq\ and the file explorer opens.\\
The user chooses the neural network/s.\\
The user can choose between three operating modes:\\
\grqq HighPerformance\grqq, \grqq HighEffiency\grqq\ and \grqq LowPower\grqq.\\
...
\caption{Usecase of the object detection page}
\end{figure}
The interaction with the software is the same as in the use case with classification. \\
Only the result differs, as the \glspl{bb} are also calculated.
\newpage
