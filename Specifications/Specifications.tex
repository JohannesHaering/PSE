...
& The software can take multiple \glspl{img} as input and returns the possiblities for each class for each predefined \gls{ic}. The prediction is based on a pretrained \gls{nn}.\\
& \\
...
& A \gls{nn} is used to classify \gls{img}. The result is a list of prohabilities per image class.\\
...
& \textbf{Reaction:} The dropdown Menu shows the selection \glqq Single Images\grqq. The File explorer is set to show .jpg, .png and .bmp files\\
...
& \textbf{Reaction:} The software loads the specified \glspl{img}.\\
...
& \textbf{Reaction:} The software calculates all images and shows one image with its result. Arrows are shown for the user to navigate the results as shown in figure 6. \\
\end{tabular}
\newpage
